...
public class GestorCliente {
    ...
    public String clientesEliminados(Model model, @RequestParam(name = "dui", required = false) Long duiNumero){
        List<Cliente> clientes;

        String dui = null;
        if (duiNumero != null) {
            // Si se proporciona un dui numérico, conviértelo a una cadena
            dui = Long.toString(duiNumero);
            clientes = clienteDao.findByDuiStartingWithAndEstadoFalse(dui);
    ...
    public String buscarNombreFechaCliente(@RequestParam(name = "nombre", required = false) String nombre,
                                        ...
                                        @RequestParam(name = "dui", required = false) Long duiNumero,
                                        Model model) {
        ...
        //Casteo para poder trabajar con dui como String
        String dui = null;
        if (duiNumero != null) {
            // Si se proporciona un dui numérico, conviértelo a una cadena
            dui = Long.toString(duiNumero);
        ...
        if (nombre != null && !nombre.isEmpty() && dui != null && (fechaDesde != null || fechaHasta != null)) {
            ...
            for (Orden orden : ordenes) {
                if (orden.getVehiculo().getCliente().getNombre().startsWith(nombre)) {
                    if (orden.getVehiculo().getCliente().getDui().startsWith(dui)) {
        ...
        }else if (nombre != null && !nombre.isEmpty() && dui == null && (fechaDesde != null || fechaHasta != null)) {
            List<Orden> ordenes = ordenService.listarOrdenesFecha(fechaDesde, fechaHasta);
        ...
        }else if (nombre.isEmpty()  && dui != null && (fechaDesde != null || fechaHasta != null)) {
            List<Orden> ordenes = ordenService.listarOrdenesFecha(fechaDesde, fechaHasta);
            ...
            clientesSet.addAll(clienteService.buscarClienteDui(dui));
        }else {
